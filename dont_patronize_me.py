...
class DontPatronizeMe:
	...
	def load_task1(self):
		...
		"""
		rows=[]
		with open(os.path.join(self.train_path, 'data/dontpatronizeme_pcl.tsv')) as f:
	...
	def load_task2(self, return_one_hot=True):
		...
		with open (os.path.join(self.train_path, 'data/dontpatronizeme_categories.tsv')) as f:
			for line in f.readlines()[4:]:
				par_id=line.strip().split('\t')[0]
				art_id = line.strip().split('\t')[1]
